...
# This Code is mostly the same as Code Memory_B_cell_COVID_Preprocessing.R.
# The difference is in part 7, where Segment Usage is compared between MBC subsets using edgeR.
...
combined <- combineBCR(contig.list,
                       samples = c("PFCL1-179308-1988_2", "PFCL1-179308-1988_3", "PFCL1-199474-1994_2", "PFCL1-199474-1994_3", # PFCL1-179308-198_2 should be PFCL1-179308-1988_2
...
# Use of the vizGenes function to compare the gene usages of positive and negative cells.
i <- 1
...
Bmem <- Bmem.backup
Bmem <- subset(Bmem, subset=sample_group=="12 months vaccinated")

trace(vizGenes, edit = T) # Change output to df instead of plot - needs to be change the first time of every session.

# Create the cell barcodes used for each column in edgeR. This will be 6(Patients)x3(Subsets)x2(Binding / Non Binding) = 36 columns
Bmem@meta.data$named.clusters <- as.character(Bmem@meta.data$named.clusters)
Bmem@meta.data$named.clusters[Bmem@meta.data$named.clusters=="CD27high RM" | Bmem@meta.data$named.clusters=="CD27low RM"] <- "RM"

PFCL1_LIM_313000.binding.RM <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-LIM-313000" & Bmem@meta.data$named.clusters=="RM" & Bmem@meta.data$bait.positive=="yes",])
PFCL1_LIM_313000.binding.Activated <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-LIM-313000" & Bmem@meta.data$named.clusters=="Activated" & Bmem@meta.data$bait.positive=="yes",])
PFCL1_LIM_313000.binding.Atypical <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-LIM-313000" & Bmem@meta.data$named.clusters=="Atypical" & Bmem@meta.data$bait.positive=="yes",])
PFCL1_LIM_313000.non.binding.RM <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-LIM-313000" & Bmem@meta.data$named.clusters=="RM" & Bmem@meta.data$bait.positive=="no",])
PFCL1_LIM_313000.non.binding.Activated <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-LIM-313000" & Bmem@meta.data$named.clusters=="Activated" & Bmem@meta.data$bait.positive=="no",])
PFCL1_LIM_313000.non.binding.Atypical <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-LIM-313000" & Bmem@meta.data$named.clusters=="Atypical" & Bmem@meta.data$bait.positive=="no",])

PFCL1_LIM_137402.binding.RM <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-LIM-137402" & Bmem@meta.data$named.clusters=="RM" & Bmem@meta.data$bait.positive=="yes",])
PFCL1_LIM_137402.binding.Activated <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-LIM-137402" & Bmem@meta.data$named.clusters=="Activated" & Bmem@meta.data$bait.positive=="yes",])
PFCL1_LIM_137402.binding.Atypical <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-LIM-137402" & Bmem@meta.data$named.clusters=="Atypical" & Bmem@meta.data$bait.positive=="yes",])
PFCL1_LIM_137402.non.binding.RM <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-LIM-137402" & Bmem@meta.data$named.clusters=="RM" & Bmem@meta.data$bait.positive=="no",])
PFCL1_LIM_137402.non.binding.Activated <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-LIM-137402" & Bmem@meta.data$named.clusters=="Activated" & Bmem@meta.data$bait.positive=="no",])
PFCL1_LIM_137402.non.binding.Atypical <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-LIM-137402" & Bmem@meta.data$named.clusters=="Atypical" & Bmem@meta.data$bait.positive=="no",])

PFCL1_LIM828246.binding.RM <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-LIM828246" & Bmem@meta.data$named.clusters=="RM" & Bmem@meta.data$bait.positive=="yes",])
PFCL1_LIM828246.binding.Activated <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-LIM828246" & Bmem@meta.data$named.clusters=="Activated" & Bmem@meta.data$bait.positive=="yes",])
PFCL1_LIM828246.binding.Atypical <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-LIM828246" & Bmem@meta.data$named.clusters=="Atypical" & Bmem@meta.data$bait.positive=="yes",])
PFCL1_LIM828246.non.binding.RM <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-LIM828246" & Bmem@meta.data$named.clusters=="RM" & Bmem@meta.data$bait.positive=="no",])
PFCL1_LIM828246.non.binding.Activated  <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-LIM828246" & Bmem@meta.data$named.clusters=="Activated" & Bmem@meta.data$bait.positive=="no",])
PFCL1_LIM828246.non.binding.Atypical <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-LIM828246" & Bmem@meta.data$named.clusters=="Atypical" & Bmem@meta.data$bait.positive=="no",])

PFCL1_199474_1994.binding.RM <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-199474-1994" & Bmem@meta.data$named.clusters=="RM" & Bmem@meta.data$bait.positive=="yes",])
PFCL1_199474_1994.binding.Activated <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-199474-1994" & Bmem@meta.data$named.clusters=="Activated" & Bmem@meta.data$bait.positive=="yes",])
PFCL1_199474_1994.binding.Atypical <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-199474-1994" & Bmem@meta.data$named.clusters=="Atypical" & Bmem@meta.data$bait.positive=="yes",])
PFCL1_199474_1994.non.binding.RM <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-199474-1994" & Bmem@meta.data$named.clusters=="RM" & Bmem@meta.data$bait.positive=="no",])
PFCL1_199474_1994.non.binding.Activated <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-199474-1994" & Bmem@meta.data$named.clusters=="Activated" & Bmem@meta.data$bait.positive=="no",])
PFCL1_199474_1994.non.binding.Atypical <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-199474-1994" & Bmem@meta.data$named.clusters=="Atypical" & Bmem@meta.data$bait.positive=="no",])

PFCL1_179308_1988.binding.RM <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-179308-1988" & Bmem@meta.data$named.clusters=="RM" & Bmem@meta.data$bait.positive=="yes",])
PFCL1_179308_1988.binding.Activated <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-179308-1988" & Bmem@meta.data$named.clusters=="Activated" & Bmem@meta.data$bait.positive=="yes",])
PFCL1_179308_1988.binding.Atypical <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-179308-1988" & Bmem@meta.data$named.clusters=="Atypical" & Bmem@meta.data$bait.positive=="yes",])
PFCL1_179308_1988.non.binding.RM <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-179308-1988" & Bmem@meta.data$named.clusters=="RM" & Bmem@meta.data$bait.positive=="no",])
PFCL1_179308_1988.non.binding.Activated <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-179308-1988" & Bmem@meta.data$named.clusters=="Activated" & Bmem@meta.data$bait.positive=="no",])
PFCL1_179308_1988.non.binding.Atypical <- rownames(Bmem@meta.data[Bmem@meta.data$Patient=="PFCL1-179308-1988" & Bmem@meta.data$named.clusters=="Atypical" & Bmem@meta.data$bait.positive=="no",])

vectors <- list(PFCL1_LIM_313000.binding.RM,PFCL1_LIM_313000.binding.Activated,PFCL1_LIM_313000.binding.Atypical,PFCL1_LIM_313000.non.binding.RM,PFCL1_LIM_313000.non.binding.Activated,PFCL1_LIM_313000.non.binding.Atypical,
             PFCL1_LIM_137402.binding.RM,PFCL1_LIM_137402.binding.Activated,PFCL1_LIM_137402.binding.Atypical,PFCL1_LIM_137402.non.binding.RM,PFCL1_LIM_137402.non.binding.Activated,PFCL1_LIM_137402.non.binding.Atypical,
             PFCL1_LIM828246.binding.RM,PFCL1_LIM828246.binding.Activated,PFCL1_LIM828246.binding.Atypical,PFCL1_LIM828246.non.binding.RM,PFCL1_LIM828246.non.binding.Activated,PFCL1_LIM828246.non.binding.Atypical,
             PFCL1_199474_1994.binding.RM,PFCL1_199474_1994.binding.Activated,PFCL1_199474_1994.binding.Atypical,PFCL1_199474_1994.non.binding.RM,PFCL1_199474_1994.non.binding.Activated,PFCL1_199474_1994.non.binding.Atypical,
             PFCL1_179308_1988.binding.RM,PFCL1_179308_1988.binding.Activated,PFCL1_179308_1988.binding.Atypical,PFCL1_179308_1988.non.binding.RM,PFCL1_179308_1988.non.binding.Activated,PFCL1_179308_1988.non.binding.Atypical)

names(vectors) <- c('PFCL1_LIM_313000.binding.RM','PFCL1_LIM_313000.binding.Activated','PFCL1_LIM_313000.binding.Atypical','PFCL1_LIM_313000.non.binding.RM','PFCL1_LIM_313000.non.binding.Activated','PFCL1_LIM_313000.non.binding.Atypical',
                     'PFCL1_LIM_137402.binding.RM','PFCL1_LIM_137402.binding.Activated','PFCL1_LIM_137402.binding.Atypical','PFCL1_LIM_137402.non.binding.RM','PFCL1_LIM_137402.non.binding.Activated','PFCL1_LIM_137402.non.binding.Atypical',
                     'PFCL1_LIM828246.binding.RM','PFCL1_LIM828246.binding.Activated','PFCL1_LIM828246.binding.Atypical','PFCL1_LIM828246.non.binding.RM','PFCL1_LIM828246.non.binding.Activated','PFCL1_LIM828246.non.binding.Atypical',
                     'PFCL1_199474_1994.binding.RM','PFCL1_199474_1994.binding.Activated','PFCL1_199474_1994.binding.Atypical','PFCL1_199474_1994.non.binding.RM','PFCL1_199474_1994.non.binding.Activated','PFCL1_199474_1994.non.binding.Atypical',
                     'PFCL1_179308_1988.binding.RM','PFCL1_179308_1988.binding.Activated','PFCL1_179308_1988.binding.Atypical','PFCL1_179308_1988.non.binding.RM','PFCL1_179308_1988.non.binding.Activated','PFCL1_179308_1988.non.binding.Atypical')

# Getting the contigs for each of them
i <- 1
vectors.contigs <- list()
for (i in 1:length(vectors)) {
  vectors.contigs[[i]] <- as.data.frame(df[df$barcode %in% vectors[[i]],])
  i <- i+1
}


# Sample information is wrong because of errors I did earler in the script (lines ). The mistake that I did leads to wrong sample ID entries in the "combined" files.
# Having wrong sample IDs in the combined files leads to list entries with more than one sample ID in the vector.contigs list object, which then causes further mistakes down the line when creating the final 
# matrix for edgeR (duplicate row names arise).
# Solution: Rename the samples in the vector.contigs to the correct names
i <- 1
for (i in 1:length(vectors.contigs)) {
  vectors.contigs[[i]]$sample <- names(vectors[i])
}


# Applying the VizGenes function for each of them. Here you need to change to the chain to be looked at!
i <- 1
vizGenes.list <- list()
for (i in 1:length(vectors.contigs)) {
  vizGenes.list[[i]] <- vizGenes(list(vectors.contigs[[i]]), gene = "V", chain = "IGL", plot = "bar", order = "variance", scale = T)
}

vizGenes.list <- lapply(vizGenes.list, function(x) { x$absolute <- x$mean*x$sum; x })


# Getting all segments found
i <- 1
collect.segments <- c()
for (i in 1:length(vizGenes.list)) {
  collect.segments <- c(collect.segments,as.character(vizGenes.list[[i]]$Var1))
}
collect.segments <- unique(collect.segments)


# Creating the final data frame
df <- as.data.frame(matrix(0, nrow = length(collect.segments), ncol = length(vizGenes.list)))
rownames(df) <- collect.segments
colnames(df) <- names(vectors)

i <- 1
for (i in 1:length(vizGenes.list)) {
  thisrounds.df <- as.data.frame(vizGenes.list[[i]][,c("Var1","absolute")])
  rownames(thisrounds.df) <- thisrounds.df$"Var1"
  df<- merge(df,thisrounds.df,by=0,all.x=T)
...
# Preparing the colData
colData <- as.data.frame(matrix(0, nrow = ncol(df), ncol = 3))
rownames(colData) <- colnames(df)
colnames(colData) <- c("Patient","Subset","Binder")
colData$Patient <- str_extract(rownames(colData), "[^.]+")
colData$Subset <- "RM"
colData$Subset[grep("Activated",rownames(colData))] <- "Activated"
colData$Subset[grep("Atypical",rownames(colData))] <- "Atypical"
colData$Binder <- "non.binding"
colData$Binder[c(1,2,3,7,8,9,13,14,15,19,20,21,25,26,27)] <- "binding"

# Set up edgeR object
y <- DGEList(counts=count.table,samples=colData)

# Specify the model (take out "Binder") when looking at the subsetted data
mdl <- model.matrix(~ 0 + Subset + Patient + Binder,y$samples)

#Remove lowly expressed segments
keep <- filterByExpr(y, design=mdl,min.count=0,min.total.count=15)
y <- y[keep,]

# Normalizaiton
y <- calcNormFactors(y,method="TMMwsp")

# Follow the standard edgeR workflow to fit a model
y <- estimateDisp(y, mdl)
fit <- glmQLFit(y, mdl, robust=TRUE)

# Do the test and plot
res <- glmQLFTest(fit, coef="Bindernon.binding")

top <- topTags(res,n=nrow(y))$table
top$Gene <- rownames(top)
ggplot(top, aes(x=logFC, y=-log10(FDR))) + geom_point(size=.5,color="grey80") + geom_point(data=top[top$FDR<0.10,], size=1.5)+
  geom_text_repel(data=top[top$FDR<0.10,], aes(label=Gene),size=3) + theme(text=element_text(size=12)) + xlab("logFC") + ylab("-log10(FDR)") + theme_bw()

# This is the code used to generate the plots from the Extended data.
# Modify the above to the the comparison that is done in the heatmap -> Unify non binding RM, Atypical and Activated
count.table.subset <- as.data.frame(count.table)
count.table.subset$PFCL1_LIM_313000.non.binding <- count.table.subset[,"PFCL1_LIM_313000.non.binding.RM"]+count.table.subset[,"PFCL1_LIM_313000.non.binding.Activated"]+count.table.subset[,"PFCL1_LIM_313000.non.binding.Atypical"]
count.table.subset$PFCL1_LIM_137402.non.binding <- count.table.subset[,"PFCL1_LIM_137402.non.binding.RM"]+count.table.subset[,"PFCL1_LIM_137402.non.binding.Activated"]+count.table.subset[,"PFCL1_LIM_137402.non.binding.Atypical"]
count.table.subset$PFCL1_LIM828246.non.binding <- count.table.subset[,"PFCL1_LIM828246.non.binding.RM"]+count.table.subset[,"PFCL1_LIM828246.non.binding.Activated"]+count.table.subset[,"PFCL1_LIM828246.non.binding.Atypical"]
count.table.subset$PFCL1_199474_1994.non.binding <- count.table.subset[,"PFCL1_199474_1994.non.binding.RM"]+count.table.subset[,"PFCL1_199474_1994.non.binding.Activated"]+count.table.subset[,"PFCL1_199474_1994.non.binding.Atypical"]
count.table.subset$PFCL1_179308_1988.non.binding <- count.table.subset[,"PFCL1_179308_1988.non.binding.RM"]+count.table.subset[,"PFCL1_179308_1988.non.binding.Activated"]+count.table.subset[,"PFCL1_179308_1988.non.binding.Atypical"]

count.table.subset <- count.table.subset[,c(1,2,3,7,8,9,13,14,15,19,20,21,25,26,27,31,32,33,34,35)]
count.table.subset <- as.matrix(count.table.subset)

colData.subset <- as.data.frame(matrix(0, nrow = ncol(count.table.subset), ncol = 3))
rownames(colData.subset) <- colnames(count.table.subset)
colnames(colData.subset) <- c("Patient","Subset","Binder")
colData.subset$Patient <- str_extract(rownames(colData.subset), "[^.]+")
colData.subset$Subset <- "RM"
colData.subset$Subset[grep("Activated",rownames(colData.subset))] <- "Activated"
colData.subset$Subset[grep("Atypical",rownames(colData.subset))] <- "Atypical"
colData.subset$Subset[c(16:20)] <- "Non_Binder"
colData.subset$Binder <- "Binding"
colData.subset$Binder[c(16:20)] <- "Non.Binding"

y <- DGEList(counts=count.table.subset,samples=colData.subset)
mdl <- model.matrix(~ 0 + Subset + Patient,y$samples)
keep <- filterByExpr(y, design=mdl,min.count=0,min.total.count=10)
y <- y[keep,]
y <- calcNormFactors(y,method="TMMwsp")
y <- estimateDisp(y, mdl)
fit <- glmQLFit(y, mdl, robust=TRUE)
res <- glmQLFTest(fit, coef="PatientPFCL1_LIM828246")
top <- topTags(res,n=nrow(y))$table
top$Gene <- rownames(top)
ggplot(top, aes(x=logFC, y=-log10(FDR))) + geom_point(size=.5,color="grey80") + geom_point(data=top[top$FDR<0.10,], size=1.5)+
  geom_text_repel(data=top[top$FDR<0.10,], aes(label=Gene),size=3) + theme(text=element_text(size=12)) + xlab("logFC") + ylab("-log10(FDR)") + theme_bw()

con <- makeContrasts(levels=colnames(mdl),"SubsetNon_Binder-SubsetRM")
res <- glmQLFTest(fit, contrast=con)
top <- topTags(res,n=nrow(y))$table
top$Gene <- rownames(top)
top$asterics <- ""
asterics.function.3 <- function(prop.test.result) {
  i <- 1
  for (i in 1:nrow(prop.test.result)) {
    
  if(prop.test.result[i,4]>0.05 | prop.test.result[i,5]>=0.1){
    prop.test.result[i,7] <- "ns"
  }
  if(prop.test.result[i,4]<=0.05 & prop.test.result[i,5]<0.1){
    prop.test.result[i,7] <- "*"
  }
  if(prop.test.result[i,4]<=0.01 & prop.test.result[i,5]<0.1){
    prop.test.result[i,7] <- "**"
  }
  if(prop.test.result[i,4]<=0.001 & prop.test.result[i,5]<0.1){
    prop.test.result[i,7]  <- "***"
  }
  if(prop.test.result[i,4]<=0.0001 & prop.test.result[i,5]<0.1){
    prop.test.result[i,7] <- "****"
  }
    i <- i+1
  } 
...
top <- asterics.function.3(top)
write.csv(top,"SubsetNon_Binder-SubsetRM_LC.csv")
ggplot(top, aes(x=logFC, y=-log10(FDR))) +
  geom_point(size=.5,color="grey80") +
  geom_point(data=top[top$FDR<0.1 ,], size=1.5)+
  geom_text_repel(data=top[top$FDR<0.1 ,], aes(label=Gene),size=3) +
  theme(text=element_text(size=12)) +
  xlab("logFC") +
  ylab("-log10(FDR)") +
  theme_bw()

con <- makeContrasts(levels=colnames(mdl),"SubsetNon_Binder-SubsetActivated")
res <- glmQLFTest(fit, contrast=con)
top <- topTags(res,n=nrow(y))$table
top$Gene <- rownames(top)
top$asterics <- ""
top <- asterics.function.3(top)
write.csv(top,"SubsetNon_Binder-SubsetActivated_LC.csv")
ggplot(top, aes(x=logFC, y=-log10(FDR))) +
  geom_point(size=.5,color="grey80") +
  geom_point(data=top[top$FDR<0.1 ,], size=1.5)+
  geom_text_repel(data=top[top$FDR<0.1 ,], aes(label=Gene),size=3) +
...
con <- makeContrasts(levels=colnames(mdl),"SubsetNon_Binder-SubsetAtypical")
res <- glmQLFTest(fit, contrast=con)
top <- topTags(res,n=nrow(y))$table
top$Gene <- rownames(top)
top$asterics <- ""
top <- asterics.function.3(top)
write.csv(top,"SubsetNon_Binder-SubsetAtypical_LC.csv")
ggplot(top, aes(x=logFC, y=-log10(FDR))) +
  geom_point(size=.5,color="grey80") +
  geom_point(data=top[top$FDR<0.1 ,], size=1.5)+
  geom_text_repel(data=top[top$FDR<0.1 ,], aes(label=Gene),size=3) +
...
con <- makeContrasts(levels=colnames(mdl),"SubsetAtypical-SubsetActivated")
res <- glmQLFTest(fit, contrast=con)
top <- topTags(res,n=nrow(y))$table
top$Gene <- rownames(top)
top$asterics <- ""
top <- asterics.function.3(top)
write.csv(top,"SubsetAtypical-SubsetActivated_LC.csv")
ggplot(top, aes(x=logFC, y=-log10(FDR))) +
  geom_point(size=.5,color="grey80") +
  geom_point(data=top[top$FDR<0.1 ,], size=1.5)+
  geom_text_repel(data=top[top$FDR<0.1 ,], aes(label=Gene),size=3) +
...
con <- makeContrasts(levels=colnames(mdl),"SubsetAtypical-SubsetRM")
res <- glmQLFTest(fit, contrast=con)
top <- topTags(res,n=nrow(y))$table
top$Gene <- rownames(top)
top$asterics <- ""
top <- asterics.function.3(top)
write.csv(top,"SubsetAtypical-SubsetRM_LC.csv")
ggplot(top, aes(x=logFC, y=-log10(FDR))) +
  geom_point(size=.5,color="grey80") +
  geom_point(data=top[top$FDR<0.1 ,], size=1.5)+
  geom_text_repel(data=top[top$FDR<0.1 ,], aes(label=Gene),size=3) +
...
con <- makeContrasts(levels=colnames(mdl),"SubsetActivated-SubsetRM")
res <- glmQLFTest(fit, contrast=con)
top <- topTags(res,n=nrow(y))$table
top$Gene <- rownames(top)
top$asterics <- ""
top <- asterics.function.3(top)
write.csv(top,"SubsetActivated-SubsetRM_LC.csv")
ggplot(top, aes(x=logFC, y=-log10(FDR))) +
  geom_point(size=.5,color="grey80") +
  geom_point(data=top[top$FDR<0.1 ,], size=1.5)+
  geom_text_repel(data=top[top$FDR<0.1 ,], aes(label=Gene),size=3) +
...
positives_RM <- vizGenes(df_positives_list_RM, gene = "V", chain = "IGH", plot = "bar", order = "variance", scale = T)
positives_Activated <- vizGenes(df_positives_list_Activated, gene = "V", chain = "IGH", plot = "bar", order = "variance", scale = T)
positives_Atypical <- vizGenes(df_positives_list_Atypical, gene = "V", chain = "IGH", plot = "bar", order = "variance", scale = T)
negatives <- vizGenes(df_negatives_list, gene = "V", chain = "IGH", plot = "bar", order = "variance", scale = T)

totalforstats_positives_RM <- unique(positives_RM$sum)
totalforstats_positives_Activated <- unique(positives_Activated$sum)
totalforstats_positives_Atypical <- unique(positives_Atypical$sum)
totalforstats_negatives <- unique(negatives$sum)

positives_RM <- unique(positives_RM[, c("Var1", "Var2", "sd", "mean")])
positives_Activated <- unique(positives_Activated[, c("Var1", "Var2", "sd", "mean")])
positives_Atypical <- unique(positives_Atypical[, c("Var1", "Var2", "sd", "mean")])
negatives <- unique(negatives[, c("Var1", "Var2", "sd", "mean")])

positives_RM$mean <- positives_RM$mean*100
positives_Activated$mean <- positives_Activated$mean*100
positives_Atypical$mean <- positives_Atypical$mean*100
negatives$mean <- negatives$mean*100

positives_RM$Set <- "RM RBD binder"
positives_Activated$Set <- "Activated RBD binder"
positives_Atypical$Set <- "Atypical RBD binder"
negatives$Set <- "Non binder"

# The following is done because there are some segments which are never used in the binders - those we are now adding manually with mean usage 0.
adder_RM <- as.data.frame(negatives[negatives$Var1 %notin% positives_RM$Var1,])
adder_Activated <- as.data.frame(negatives[negatives$Var1 %notin% positives_Activated$Var1,])
adder_Atypical <- as.data.frame(negatives[negatives$Var1 %notin% positives_Atypical$Var1,])
adder_RM$mean <- 0
adder_Activated$mean <- 0
adder_Atypical$mean <- 0
adder_RM$Set <- "RM RBD binder"
adder_Activated$Set <- "Activated RBD binder"
adder_Atypical$Set <- "Atypical RBD binder"
total <- rbind(positives_RM,adder_RM,positives_Activated,adder_Activated,positives_Atypical,adder_Atypical,negatives)
...
ggplot(total, aes(x = Var1, y = mean, fill=Set)) + geom_bar(stat = "identity",position=position_dodge()) + 
  ...
  scale_fill_manual(values=met.brewer("Austria", 4))+ggtitle("V Heavy Frequency (%)")+
  scale_x_discrete(expand = c(0, 0))+scale_y_continuous(expand = c(0,0) )

# Show results as heatmap
total <- total[,c(1,4,5)]
total <- as.data.frame(total)
total_Binders_RM <- total[total$Set=="RM RBD binder",]
total_Binders_Activated <- total[total$Set=="Activated RBD binder",]
total_Binders_Atypical <- total[total$Set=="Atypical RBD binder",]
total_non_Binders <- total[total$Set=="Non binder",]

rownames(total_Binders_RM) <- total_Binders_RM$Var1
rownames(total_Binders_Activated) <- total_Binders_Activated$Var1
rownames(total_Binders_Atypical) <- total_Binders_Atypical$Var1
rownames(total_non_Binders) <- total_non_Binders$Var1

total_Binders_RM <- total_Binders_RM[,c("mean","Set")]
total_Binders_Activated <- total_Binders_Activated[,c("mean","Set")]
total_Binders_Atypical <- total_Binders_Atypical[,c("mean","Set")]
total_non_Binders <- total_non_Binders[,c("mean","Set")]

total.new <- merge(total_Binders_Activated, total_Binders_RM, all.x = T, by=0)
rownames(total.new) <- total.new$Row.names
total.new$Row.names <- NULL
total.new$Set.x <- NULL
total.new$Set.y <- NULL
colnames(total.new) <- c("Activated","RM")
total.new <- merge(total.new, total_Binders_Atypical, all.x = T, by=0)
rownames(total.new) <- total.new$Row.names
total.new$Row.names <- NULL
total.new$Set <- NULL
colnames(total.new) <- c("Activated","RM","Atypical")
total.new <- merge(total.new, total_non_Binders, all.x = T, by=0)
rownames(total.new) <- total.new$Row.names
total.new$Row.names <- NULL
total.new$Set <- NULL
colnames(total.new) <- c("Activated","RM","Atypical","Non_Binder")

total.new$RM[is.na(total.new$RM)] <- 0
total.new$Atypical[is.na(total.new$Atypical)] <- 0
total.new$Non_Binder[is.na(total.new$Non_Binder)] <- 0
total.new.scaled <-myscaling(total.new)

# Subset to only top 30 RBD binding segments
total.new <- total.new[order(total.new$RM, decreasing = T),]
total.new <- total.new[1:20,]
total.new <- total.new[,c(2,1,3,4)]
...
cairo_pdf("Gene_usage_heavy_chain_subsets.pdf",width = 6, height = 15)
pheatmap(total.new,color =  inferno(80),cutree_rows = 3,cluster_cols=F,fontsize_col=12,
