...
# Anonymization of Patient IDs for Final ExtendedFig8:
# PFCL1-154583-1943 -> CovSeq_Pat06
# PFCL1-179308-1988 -> CovSeq_Pat07
# PFCL1-196878-1691 -> CovSeq_Pat03
# PFCL1-199474-1994 -> CovSeq_Pat09
# PFCL1-LIM-137402 -> CovSeq_Pat05
# PFCL1-LIM-313000 -> CovSeq_Pat04
# PFCL1-LIM-674950 -> CovSeq_Pat01
# PFCL1-LIM828246 -> CovSeq_Pat08
# PFCL1-UST-190762 -> CovSeq_Pat02

charactervector <- c("CovSeq_Pat06","CovSeq_Pat07","CovSeq_Pat03","CovSeq_Pat09","CovSeq_Pat05","CovSeq_Pat04","CovSeq_Pat01","CovSeq_Pat08","CovSeq_Pat02")
...
Patients.df$Patient <- factor(Patients.df$Patient, levels=c("CovSeq_Pat06","CovSeq_Pat07","CovSeq_Pat03","CovSeq_Pat09","CovSeq_Pat05","CovSeq_Pat04","CovSeq_Pat01","CovSeq_Pat08","CovSeq_Pat02"))
#Patients.df$Patient <- as.character(Patients.df$Patient)
ggplot(Patients.df, 
...
Bait.positive.markers.overtime <- Bait.positive.markers.overtime[Bait.positive.markers.overtime$cluster=="12 months",]
...
# Differential gene expression between positive cells 6mo vs 12 mo not vaccinated. All cells. And only wt spike binders- Fig.2.I
Bait.positives <- subset(Bmem, subset = cor_wt_Spike > 0 & Dataset!= "Second")
Bait.positives <- subset(Bait.positives, subset = Vaccinated =="no")
Bait.positives@meta.data$Timepoint[Bait.positives@meta.data$Timepoint=="12 months after infection"] <- "12 months"
Bait.positives@meta.data$Timepoint[Bait.positives@meta.data$Timepoint=="6 months after infection"] <- "6 months"
Idents(Bait.positives) <- "Timepoint"
...
Bait.positive.markers.overtime <- FindAllMarkers(Bait.positives, assay = "RNA", logfc.threshold = 0.1,min.pct = 0.1)
Bait.positive.markers.overtime <- Bait.positive.markers.overtime[Bait.positive.markers.overtime$cluster=="6 months",]
rownames(Bait.positive.markers.overtime) <- Bait.positive.markers.overtime$gene
Bait.positive.markers.overtime <- Bait.positive.markers.overtime[Bait.positive.markers.overtime$p_val_adj<0.05,]
Bait.positive.markers.overtime <- Bait.positive.markers.overtime[Bait.positive.markers.overtime$avg_log2FC>0,]

# Dotplot
DotPlot(Bait.positives, features = c("CXCR4","CD83","NFKBIA","NFKBID","NR4A2","MAP3K8","JUND","BCL10","MS4A1","PTPN6",
                                     "IL2RG","TLR10","CD53","BCL6","AICDA"),assay = "RNA",scale = F,cols=c("white","dodgerblue3")) + RotatedAxis()+labs(x="",y="")+theme_classic()+
...
DotPlot(Bait.positives, features = c("CXCR4","CD83","NFKBIA","NFKBID","NR4A2","MAP3K8","JUND","BCL10","MS4A1",
                                     "CD79B","PTPN6","IL2RG","TLR10","CD53"),assay = "RNA",scale = T) + RotatedAxis()+labs(x="",y="")+theme_classic()+
  theme(legend.position = "right",text = element_text(size = 20),axis.text.x = element_text(angle = 45, hjust=1))+
  black.axis.text() # Export 5x11

write.xlsx(Bait.positive.markers.overtime, "DGEA_6mo_vs_12mo.xlsx")
#("CXCR4","CD83","NFKBIA","NFKBID","NR4A2","MAP3K8","JUND","BCL10","MS4A1","PTPN6",
#"IL2RG","TLR10","CD53","HLA-DQA2","RPS29","MT-ATP8","RPS10","MT-ATP6","IGHG2")
# Volcano Plot
EnhancedVolcano(Bait.positive.markers.overtime,
                lab = rownames(Bait.positive.markers.overtime),
                x = 'avg_log2FC',
                y = 'p_val_adj',
                title = '6 months vs. 12 months',
                pCutoff = 0.05,
                FCcutoff = 0.3,
                selectLab = c("CXCR4","CD83","NFKBIA","NFKBID","NR4A2","MAP3K8","JUND","BCL10","MS4A1","PTPN6",
                              "IL2RG","TLR10","CD53","HLA-DQA2","RPS29","MT-ATP8","RPS10","MT-ATP6","IGHG2"),
                xlim = c(-2, 2),
                boxedLabels = F,
                drawConnectors = TRUE,
                legendPosition = 'none',arrowheads = F,labCol="gray32",
                subtitle="",caption="", labSize=4, col = c("black", "black", "black", "red2"))+center.title() # 5x6

# Reviewer 2 major concern 2
genes <- c("BCL6", "AICDA", "CXCR4", "IL21R")
Bait.positives@meta.data$Timepoint <- factor(Bait.positives@meta.data$Timepoint, levels = c("6 months","12 months"))
Idents(Bait.positives) <- "Timepoint"
a <- VlnPlot(Bait.positives,assay = "RNA",features = "CXCR4",cols=c("#004488","#997700"))& theme(axis.title.x = element_blank())
b <- VlnPlot(Bait.positives,assay = "RNA",features = "BCL6",cols=c("#004488","#997700"))& theme(axis.title.x = element_blank())
c <- VlnPlot(Bait.positives,assay = "RNA",features = "IL21R",cols=c("#004488","#997700"))& theme(axis.title.x = element_blank())
d <- VlnPlot(Bait.positives,assay = "RNA",features = "AICDA",cols=c("#004488","#997700"))& theme(axis.title.x = element_blank())
ggarrange(a,b,c,d)
...
# Is there a difference in LIBRA scores for cells of different subsets?
Bmem.subset <- subset(Bmem, subset = bait.positive=="yes" &
                        sample_group=="12 months vaccinated")
Bmem.subset <- Bmem.subset@meta.data[,c(33,53)]
Bmem.subset$named.clusters <- factor(Bmem.subset$named.clusters, levels = c("Unswitched","CD27low RM","CD27high RM","Activated","Atypical"))
stat.test <- Bmem.subset %>% rstatix::wilcox_test(cor_wt_Spike~named.clusters)

ggplot(Bmem.subset, aes(x=named.clusters,y=cor_wt_Spike))+ 
  geom_boxplot(Bmem.subset, mapping= aes(x = named.clusters, y = cor_wt_Spike,fill=named.clusters))+
  theme_classic()+stat_pvalue_manual(stat.test, label = "p.adj", 
                                     y.position = c(1,1.1,1.2,1.3,1.5,1.6,1.7,1.8,1.9,2.1))+
  labs(x="",y="Spike LIBRA score",fill = "Cell states")+ black.axis.text()+
  scale_fill_manual(values=met.brewer(name="Egypt", n=5))+ggtitle("SARS-CoV-2-binding cells \n 12months + vaccinated")+
  center.title()# Export 5x7

remove(Bmem.subset, stat.test)
